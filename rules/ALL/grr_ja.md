...
こちらでは，Intelligent Home Robotics 2024 Challenge in Winter（以下，*iHR-C3*と呼ぶ）の競技の一般的なルールと規定である．
...
ただし，各チームは，例えば観客の動きによってロボットの認識システムに影響が出るなど，*アリーナ (Arena)*内で行われるタスクに影響を与えないための対策を考慮する必要がある．
...
*iHR-C3*の一部の競技では，オブジェクトの認識と操作が必要である．
...
1. **既知オブジェクト (Known Objects)**：ロボットが事前に知っていたオブジェクトで，以下に分類されます: 
    ...
    1. **事前公開オブジェクト (Consistent Objects)**: *Setup Day*に公開されたオブジェクト．

    1. **標準オブジェクト (Standard Objects)**: **TBD**から選ばれたオブジェクト．
これらのオブジェクトは*iHR-C3*競技会の数ヶ月前に公開され，事前に取得と学習ができるようになっている．
...
*iHR-C3*の一部の競技では，人やオブジェクトが見つかる*Predefined Location*が設定される．
...
*iHR-C3*の一部の競技では，人物の名前を記憶する必要がある．
...
*iHR-C3*に参加するロボットは，安全仕様に準拠している必要がある．
...
1. **運搬**: チームメンバーは，ロボットを迅速に*Arena*から移動する責任がある．
ロボットが自力で移動できない場合（何らかの理由で），チームメンバーはロボットを迅速かつ容易に運搬できる必要がある．
...
すべての*External devices*は，*技術委員会*（TC）によって承認される必要がある．
...
*External Computing*デバイスは，TCによって*外部コンピューティングリソースエリア*（External Computing Resource Area, ECRA）として*Setup Day*に発表される場所に配置する必要がある．
...
これらは*Setup Day*にTCに通知し，承認を得る必要がある．
...
デフォルトの*合図シグナル (start signal)*（特に明示されていない場合）は*ドアの開閉 (door opening)*となる．
ロボットはドアの後ろに待機し，*Arena*の外にいるチームメンバーと一緒にいることが求められる．
競技は，`TC`（チームメンバーではない）がドアを開けるときに開始される．
<!-- 他の合図シグナルも許可されていますが，*Robot Inspection*中に*Technical Committee*（TC）によって承認される必要があります． -->
...
*iHR-C3*では，チームは制限時間内に何度でもリスタートすることが許可されている．
...
*iHR-C3*では，*優れたパフォーマンスに対するボーナス*はない．
